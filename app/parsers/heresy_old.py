...
def parse_units(contents):
    ...
    cleaned_item_to_parse = [
        parsable_item
        for parsable_item in list_of_parsable_items
        if parsable_item != "\n"
    ]
...
def create_unit(unit_list):
    ...
    for item in unit_list:
        ...
        list_of_vehicles_in_group = item.findAll(
            "profile", {"profiletypename": "Vehicle"}
        )
        ...
        if (
            not list_of_vehicles_in_group
            and not list_of_units_in_group
            and not list_of_models_as_units
        ):
            raw_list_of_upgrades_as_units = item.find("selections").contents
            list_of_upgrades_as_units = [
                raw_list_of_upgrades_as_units
                for raw_list_of_upgrades_as_units in raw_list_of_upgrades_as_units
                if raw_list_of_upgrades_as_units.name
            ]
        ...
        parsed_list_if_units_in_group = parse_items_in_group(
            list_to_be_parsed, dict_of_abilities, dict_of_wargear
        )
        parsed_group = UnitGroup(
            name=unit_group_name, list_of_units=parsed_list_if_units_in_group
        )
...
def get_wargear(item):
    ...
    for wargear in list_of_potential_wargear:
        ...
        if hasattr(wargear.find("characteristics"), "contents"):
            ...
            cleaned_list_of_wargear_stats = [
                list_of_wargear_stats
                for list_of_wargear_stats in list_of_wargear_stats
                if list_of_wargear_stats != "\n"
            ]
...
def parse_items_in_group(list_of_units_in_group, dict_of_abilities, dict_of_wargear):
    ...
    for unit in list_of_units_in_group:
        create_unit_object(
            dict_of_abilities, parsed_units_in_group, unit, dict_of_wargear
        )
...
def create_unit_object(dict_of_abilities, parsed_units_in_group, unit, dict_of_wargear):
    ...
    cleaned_list_of_unit_characteristic = [
        list_of_unit_characteristics
        for list_of_unit_characteristics in list_of_unit_characteristics
        if list_of_unit_characteristics != "\n"
    ]

    if bool(unit.findAll("profile", {"profiletypename": "Vehicle"})):
        possible_vehicle_characteristics = unit.findAll(
            "profile", {"profiletypename": "Vehicle"}
        )[0]
        list_of_vehicle_characteristics = possible_vehicle_characteristics.find(
            "characteristics"
        ).contents
        cleaned_list_of_vehicle_characteristics = [
            list_of_vehicle_characteristics
            for list_of_vehicle_characteristics in list_of_vehicle_characteristics
    ...
    dict_of_characteristics = get_unit_characteristics(
        cleaned_list_of_unit_characteristic
    )

    parsed_unit = HeresyUnit(
        name=unit_name,
        ...
        armor_facing=ArmorFacing(
            front=dict_of_characteristics.get("Front"),
