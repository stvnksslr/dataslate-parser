...
class HeresyTest(TestCase):
    def setUp(self):
        ...
        self.night_lords_zm_force_with_vehicles = str(
            self.base_path / "test_hh_vehicles.ros"
        )
        self.single_dread_contemptor = str(self.base_path / "hh_test_dread_only.ros")
        self.single_cortus_dread_contemptor = str(
            self.base_path / "hh_test_cortus_dread_only.ros"
        )
        self.tac_squad_with_transport_only = str(
            self.base_path / "tactical_squad_with_transport.ros"
        )
        self.all_dedicated_transports = str(
            self.base_path / "all_dedicated_transports.ros"
        )
    ...
    def test__legion_tactical_squad_has_sarge_and_unit(self):
        ...
        self.assertEqual(
            parsed_units[0].list_of_units[0].name, "Legion Tactical Space Marine"
        )
        self.assertEqual(
    ...
    def test__praetor_correctly_parsed(self):
        ...
        self.assertEqual(
            parsed_units[1].list_of_units[0].unit_type, "Infantry (Character)"
        )
    ...
    def test__parse_roster_with_vehicles(self):
        parsed_roster = fetch_and_parse_roster(
            roster_file=self.night_lords_zm_force_with_vehicles, gametype=self.gametype
        )
        parsed_units = parsed_roster[1]
        self.assertTrue(parsed_units)
    ...
    def test__parse_single_dread_talon(self):
        parsed_roster = fetch_and_parse_roster(
            roster_file=self.single_dread_contemptor, gametype=self.gametype
        )
        parsed_units = parsed_roster[1]
    ...
    def test__parse_single_cortus_dread_talon(self):
        parsed_roster = fetch_and_parse_roster(
            roster_file=self.single_cortus_dread_contemptor, gametype=self.gametype
        )
        parsed_units = parsed_roster[1]
        self.assertEqual(len(parsed_units), 1)
    ...
    def test__parse_tac_squad_with_transport(self):
        parsed_roster = fetch_and_parse_roster(
            roster_file=self.tac_squad_with_transport_only, gametype=self.gametype
        )
        parsed_units = parsed_roster[1]
    ...
    def test__parse_list_with_multiple_types_of_dedicated_transport(self):
        parsed_roster = fetch_and_parse_roster(
            roster_file=self.all_dedicated_transports, gametype=self.gametype
        )
        parsed_units = parsed_roster[1]
    ...
    def test__parse_list_with_dreadclaw_dedicated_transport(self):
        parsed_roster = fetch_and_parse_roster(
            roster_file=self.dread_claw_dt, gametype=self.gametype
        )
        parsed_units = parsed_roster[1]
        self.assertEqual(len(parsed_units[0].list_of_units), 3)
    ...
    def test__parse_list_with_drop_pod_dedicated_transport(self):
        parsed_roster = fetch_and_parse_roster(
            roster_file=self.drop_pod_dt, gametype=self.gametype
        )
        parsed_units = parsed_roster[1]
    ...
    def test__parse_list_with_land_raider_dedicated_transport(self):
        parsed_roster = fetch_and_parse_roster(
            roster_file=self.phobos_dt, gametype=self.gametype
        )
        parsed_units = parsed_roster[1]
